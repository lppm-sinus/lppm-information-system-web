...
const Buku = () => {
  ...
  const fetchDataBuku = async (
    url = "/api/books/grouped-by-category",
    filter = ""
  ) => {
  ...
  const fetchDataYear = async () => {
    try {
      const res = await axios.get("/api/books/years");
      // console.log(res);
  ...
  const fetchChartData = async (url = "/api/books/chart-data", year = "") => {
  ...
  useEffect(() => {
    ...
    fetchDataBuku("/api/books/grouped-by-scheme", filter);
  }, [selectedProdi]);

  useEffect(() => {
    const year = selectedYear !== "All" ? selectedYear : "";
    fetchChartData("/api/books/chart-data", year);
  }, [selectedYear]);
