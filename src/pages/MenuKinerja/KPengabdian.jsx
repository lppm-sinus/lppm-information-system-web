...
const KPengabdian = () => {
  ...
  const fetchDataPengabdian = async (
    url = "/api/services/grouped-by-scheme",
    filter = ""
  ) => {
  ...
  const fetchDataYear = async () => {
    try {
      const res = await axios.get("/api/services/years");
      // console.log(res);
  ...
  useEffect(() => {
    ...
    fetchDataPengabdian("/api/services/grouped-by-category", filter);
  }, [selectedProdi]);

  useEffect(() => {
    const year = selectedYear !== "All" ? selectedYear : "";
    fetchChartData("/api/services/chart-data", year);
  }, [selectedYear]);
