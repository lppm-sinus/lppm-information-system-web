...
const EditPengabdian = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const amount = parseFloat(res.data.data.dana_disetujui);
      setData(res.data.data);
      setFormData({ ...res.data.data, dana_disetujui: amount });
      setSelectedAuthors(res.data.data.authors.slice(1));
      setSelectedLeader({
        nidn: res.data.data.nidn_ketua,
        name: res.data.data.nama_ketua,
      });
      setError(null);
    } catch (err) {
      console.log(err);
    ...
    <AdminPageLayout>
      <form onSubmit={handleSubmit}>
        <div className="flex justify-between  mb-5">
          <h2 className="text-xl font-bold mb-4">Edit Data Pengabdian</h2>
